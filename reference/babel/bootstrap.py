...
class SpecIndexBootstrap:
    ...
    def _discover_versions(self, start_version: str) -> List[str]:
        ...
        while True:
            ...
            # Check genesis termination FIRST (before checking if target exists)
            if current == "v0.1.0":
                # Genesis must not have basis_target
                ...
                # Genesis reached successfully
                break

            # For non-genesis, basis_target must exist
            if basis_target is None:
                # Non-genesis version with null basis_target
                raise BootstrapError(
                    code=BOOTSTRAP_MISSING_MANIFEST,
                    version=current,
            ...
            # Check if basis_target manifest exists before loading
            target_path = self.base_path / f"babel-manifest-{basis_target}.json"
            if not target_path.exists():
                # Report current version (has broken ref), not the missing target
                raise BootstrapError(
                    code=BOOTSTRAP_MISSING_MANIFEST,
                    version=current,
    ...
    def _verify_basis_chain(self, versions: List[str]) -> None:
        ...
        for i, version in enumerate(versions):
            ...
            # Check genesis termination FIRST (before any other validation)
            if version == "v0.1.0":
            ...
            # Compute hash of predecessor manifest (use Path object)
            ...
            # Check hash mismatch - use BOOTSTRAP_BASIS_MISMATCH
            # This is for well-formed basis_ref that doesn't match computed hash
